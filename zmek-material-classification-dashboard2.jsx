...
// Measured directly from cast photographs using a Python/OpenCV pipeline
// (colour/brightness-based segmentation, see project documentation).
// shrinkage = % change in detected membrane area relative to T0 (negative
// values mean the detected area grew rather than shrank -- this can
// reflect genuine swelling/unfolding or be partly a photography-distance
// artefact; both samples are measured with the same method so the
// comparison between them remains valid). fragmentCount and crackLength
// (derived from measured perimeter) are direct pipeline outputs.
// textureRoughness is the standard deviation of brightness across all
// detected membrane pixels -- higher values indicate a more visually
// varied/mottled surface.
const RAW_SAMPLES = [
  {
    id: "starch",
    ...
    note: "Brightness rises then falls (T0\u2013T3 lightening, T12\u2013T24 darkening) while fragment count spikes sharply at T24 \u2014 a two-phase, non-linear response rather than steady drying.",
    points: [
      { t: "T0", shrinkage: 0, fragmentCount: 3, crackLength: 329.9, swellingArea: 39.3, areaCm2: 1571.1, brightness: 107.4 },
      { t: "T1", shrinkage: -5.6, fragmentCount: 2, crackLength: 266.0, swellingArea: 45.1, areaCm2: 1659.3, brightness: 122.8 },
      { t: "T3", shrinkage: -4.1, fragmentCount: 2, crackLength: 262.4, swellingArea: 45.7, areaCm2: 1634.8, brightness: 135.1 },
      { t: "T12", shrinkage: -6.6, fragmentCount: 2, crackLength: 286.7, swellingArea: 46.1, areaCm2: 1675.1, brightness: 110.4 },
      { t: "T24", shrinkage: -0.1, fragmentCount: 5, crackLength: 342.3, swellingArea: 41.7, areaCm2: 1573.2, brightness: 105.1 },
    ],
  },
  {
    id: "alginate",
    ...
    note: "Brightness rises steadily and almost monotonically through T24, with fragment count and texture staying in a narrow band \u2014 a contained, gradual response rather than a sharp reversal.",
    points: [
      { t: "T0", shrinkage: 0, fragmentCount: 2, crackLength: 111.4, swellingArea: 43.6, areaCm2: 469.2, brightness: 138.8 },
      { t: "T1", shrinkage: -16.9, fragmentCount: 3, crackLength: 121.9, swellingArea: 40.3, areaCm2: 548.4, brightness: 148.4 },
      { t: "T3", shrinkage: -18.3, fragmentCount: 2, crackLength: 127.9, swellingArea: 44.3, areaCm2: 555.2, brightness: 141.2 },
      { t: "T12", shrinkage: -29.4, fragmentCount: 3, crackLength: 133.6, swellingArea: 39.7, areaCm2: 607.3, brightness: 147.2 },
      { t: "T24", shrinkage: -23.7, fragmentCount: 4, crackLength: 124.3, swellingArea: 34.9, areaCm2: 580.5, brightness: 165.1 },
    ],
  },
];
  },
...
// threshold crossing rather than fixing one in advance. Built from
// measured pipeline outputs: |area change|, perimeter (crack proxy),
// and fragment count, each normalized against the combined observed
// range across both samples.
function deformationIndex(p) {
  const areaChange = Math.abs(p.shrinkage) / 30; // measured range ~0-30%
  const c = (p.crackLength - 100) / 250; // measured range ~111-342
  const f = (p.fragmentCount - 1) / 5; // measured range 2-5
  return Math.min(1, Math.max(0, areaChange * 0.35 + c * 0.35 + f * 0.3));
}
...
function classify(sample, threshold) {
  const fragmentCounts = sample.points.map((p) => p.fragmentCount);
  const maxFragments = Math.max(...fragmentCounts);
  const minFragments = Math.min(...fragmentCounts);
  const fragmentRange = maxFragments - minFragments;
  const last = sample.points[sample.points.length - 1];
  const sharpLateSpike = last.fragmentCount === maxFragments && fragmentRange >= 2 && maxFragments >= 5;

  if (sharpLateSpike) {
    return {
      character: "Fibre-Bridged Fragmented",
      note: "Fragment count stays low and stable through mid-series readings, then spikes sharply at the final time stamp — a late, abrupt break rather than gradual fragmentation.",
    };
  }
  if (maxFragments <= 4) {
    return {
      character: "Dense Resistant Shell",
      note: "Fragment count stays within a narrow band across all time stamps — a contained, gradual response rather than a sharp break.",
    };
  }
  return {
    character: "Fast Softening Membrane",
    note: "Rapid early change with moderate, gradual deformation.",
  };
}
    ...
    >
      ...
      <div style={{ marginBottom: 28, borderBottom: "1px solid var(--line)", paddingBottom: 20 }}>
        ...
        <p style={{ fontSize: 14, color: "#54504A", marginTop: 10, maxWidth: 640, lineHeight: 1.55 }}>
          A working scaffold for the data architecture described in Stage 3 of the proposal. Every value
          below was measured from cast photographs using a Python/OpenCV pipeline (colour and brightness
          based segmentation, fragment detection, area and perimeter calculation), not estimated by eye.
          Thresholds are not fixed in advance — they are derived from each sample's own measured curve.
        </p>
      </div>
      ...
      <div className="zmek-grid" style={{ display: "grid", gridTemplateColumns: "200px 1fr", gap: 28 }}>
        ...
        <div>
          ...
          <div style={{ minHeight: 28, marginBottom: 8 }}>
            {hoveredPoint !== null && (
              <div className="zmek-mono" style={{ fontSize: 12, color: "var(--moss)" }}>
                {enriched[hoveredPoint].t} — area {enriched[hoveredPoint].areaCm2.toFixed(1)}cm² ({enriched[hoveredPoint].shrinkage >= 0 ? "-" : "+"}{Math.abs(enriched[hoveredPoint].shrinkage).toFixed(1)}%) · fragments {enriched[hoveredPoint].fragmentCount} ·
                perimeter {enriched[hoveredPoint].crackLength.toFixed(1)}cm · brightness {enriched[hoveredPoint].brightness.toFixed(1)} ·
          ...
          <div style={{ marginTop: 24 }}>
            ...
            >
              {`{ sample: "${sample.id}", t: "${enriched[enriched.length - 1].t}", areaCm2: ${enriched[enriched.length - 1].areaCm2.toFixed(1)}, shrinkage: ${enriched[enriched.length - 1].shrinkage.toFixed(1)}, fragmentCount: ${enriched[enriched.length - 1].fragmentCount}, crackLength: ${enriched[enriched.length - 1].crackLength.toFixed(1)}, brightness: ${enriched[enriched.length - 1].brightness.toFixed(1)}, deformationIndex: ${enriched[enriched.length - 1].index.toFixed(3)}, threshold: ${threshold.value.toFixed(3)}, character: "${result.character}" }`}
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ marginTop: 32, paddingTop: 16, borderTop: "1px solid var(--line)" }}>
        <p className="zmek-mono" style={{ fontSize: 11, color: "#8A857C", lineHeight: 1.6, margin: 0 }}>
          Ten rows shown: two samples × five measured time stamps, each produced by the same Python/OpenCV
          measurement pipeline. The architecture is built to scale: each additional sample and time stamp
          becomes one more row in the same structure, without changing the pipeline — this is the scaffold
          a future machine learning classifier would train on.
        </p>
      </div>
    </div>
  );
